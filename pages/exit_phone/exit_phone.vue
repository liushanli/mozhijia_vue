<template>
	<view class="login" style="margin-top: 1rem;">
		<view class="content">
			<!-- 头部logo -->
			<view class="header" style="padding-left: 1rem;">
				<h3>手机号验证</h3>
			</view>
			<!-- 主体表单 -->
			<view class="main">
			...
			<wButton 
				...
				@tap="startLogin"
			></wButton>
			<view class="action-row" style="margin-top:3rem;">
				<label style="color: #F76260;font-weight: bold;">注意：</label>
				<p>1：历史账单将无法查询；</p>
				<p>2：账号将被消除；</p>
				<p>3：放弃会员等级权益；</p>
				<p>4：放弃所存的金额，优惠券等资产；</p>											
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	//import phone from '../../components/watch-login/watch-input.vue'
	export default {
		data() {
			return {
				//logo图片 base64
		...
		methods: {
			...
			isLogin(){
				//判断缓存中是否登录过，直接登录
				const login = uni.getStorageSync("isLogin2");
				if(login){
				  uni.switchTab({
					url:"../store-home/store-home"
				  });
				}
			},
		    startLogin(e){
				...
				let url = this.$baseUrl+'/user/findUserInfoBySms?phone='+this.phoneData+"&sendCode="+this.passData;
				let that = this;
				uni.request({
				          url: url,
				          method: 'GET',
			...
			async dialogCancel(){
				...
				console.log(userVo);
				let url = this.$baseUrl+"/user/delUserInfoById?userId="+userVo.userId;
