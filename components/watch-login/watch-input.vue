...
<script>
	...
	export default{
		...
		methods:{
			...
			setCode(){
				let photoData = uni.getStorageSync("phoneData");
				...
				this.$emit('setCode');
				this.runCode(60,photoData);
			},
			runCode(val,photoData){
				console.log("----");
				//开始倒计时
				if(String(val)=="0"){
					//判断是否需要终止循环
				...
				countDown = setInterval(function(){
					if(_this.second == 60){
						uni.request({
							url:_this.$baseUrl+"/user/SmsSendMsgNew?phone="+photoData,
							method:"POST",
							success: (res) => {
								if(!res.data.success){
									uni.showToast({
